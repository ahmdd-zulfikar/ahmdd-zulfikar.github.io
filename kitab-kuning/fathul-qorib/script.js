document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentPageIndex = parseInt(localStorage.getItem('fathulQoribCurrentPage')) || 0;
    ...
    function renderPage(index) {
        ...
        if (index < 0 || index >= fathulQoribData.length) return;
        const pageData = fathulQoribData[index];
        ...
        if (isTranslationMode) {
            ...
            pageData.lines.forEach((line, i) => {
                ...
                if (line.align === 'title-box') {
                    ...
                    box.innerHTML = text.replace(/break_line/g, '<br>');
            ...
            pageData.lines.forEach(line => {
                ...
                if (line.align === 'title-box') {
                    ...
                    currentRow.style.gap = '0.35em';
                    box.appendChild(currentRow);
                    
                    line.words.forEach(w => {
                        if (w.text === 'break_line') {
                            ...
                            currentRow.appendChild(createWordEl(w));
                        }
                    });
                    lineEl.appendChild(box);
                } else {
        ...
        nextBtn.disabled = index === fathulQoribData.length - 1;
    }
    ...
    prevBtn.addEventListener('click', () => {
        if (currentPageIndex > 0) {
            currentPageIndex--;
            localStorage.setItem('fathulQoribCurrentPage', currentPageIndex);
    ...
    nextBtn.addEventListener('click', () => {
        if (currentPageIndex < fathulQoribData.length - 1) {
            currentPageIndex++;
            localStorage.setItem('fathulQoribCurrentPage', currentPageIndex);
            renderPage(currentPageIndex);
            hidePopup();
